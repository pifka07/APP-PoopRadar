...
export default function App() {
  ...
  const [stats, setStats] = useState({ 
    points: 0, total: 0, clean: 0, monster: 0, mini: 0, cityCount: 0, sizeTypes: 0, level: 1, levelName: "Gehweg-Novize"
  });
  ...
  const badgeDefinitions = [
    { id: 'first-fund', title: 'Erster Fund', subtitle: 'Ersten Haufen gemeldet', icon: '🎉', achieved: stats.total >= 1 },
    { id: 'beginner', title: 'Anfänger', subtitle: '50 Punkte erreicht', icon: '⭐', achieved: stats.points >= 50 },
    { id: 'advanced', title: 'Fortgeschritten', subtitle: '200 Punkte erreicht', icon: '🚀', achieved: stats.points >= 200 },
    { id: 'expert', title: 'Experte', subtitle: '500 Punkte erreicht', icon: '🏆', achieved: stats.points >= 500 },
    { id: 'cleaner', title: 'Saubermann', subtitle: 'Ersten Haufen weggeräumt', icon: '🧹', achieved: stats.clean >= 1 },
    { id: 'hero', title: 'Held', subtitle: '25x weggeräumt', icon: '🦸', achieved: stats.clean >= 25 },
    { id: 'eagle-eye', title: 'Adlerauge', subtitle: 'Ersten Mini-Haufen gefunden', icon: '🦅', achieved: stats.mini >= 1 },
    { id: 'elephant-hunter', title: 'Elefantenjäger', subtitle: 'Ersten Monster-Haufen gefunden', icon: '🐘', achieved: stats.monster >= 1 },
    { id: 'collector', title: 'Sammler', subtitle: 'Alle Größen gefunden', icon: '🎯', achieved: stats.sizeTypes >= 3 },
    { id: 'globetrotter', title: 'Weltenbummler', subtitle: 'In 10 verschiedenen Städten gemeldet', icon: '🌍', achieved: stats.cityCount >= 10 },
    { id: 'legend', title: 'Legende', subtitle: '1000 Punkte erreicht', icon: '👑', achieved: stats.points >= 1000 },
  ];
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      if (session) updateProfileData(session);
    });
  ...
  const updateProfileData = async (sess) => {
    if (!sess?.user) {
      setStats({ points: 0, total: 0, clean: 0, level: 1, levelName: "Gehweg-Novize", rank: '-', userCount: '-' });
      return;
    }
    ...
    try {
      ...
      if (userReports && !reportsError) {
        ...
        userReports.forEach((report) => {
          ...
          if (report?.size) sizes.add(report.size);
          if (report?.size === 'S') miniCount += 1;
          if (report?.size === 'L') monsterCount += 1;
        });
  ...
  const checkNearbyReports = async () => {
    ...
    try {
      ...
      if (lastMarkerSyncTime) {
        query = query.gt('created_at', lastMarkerSyncTime);
      }
      ...
      if (newReports && newReports.length > 0) {
        for (const report of newReports) {
          if (!report?.id || notifiedReportIds.current.has(report.id)) continue;
  ...
  const fetchAllMarkers = async () => {
    ...
    if (data) {
      ...
      const latestCreatedAt = data
        ...
        .sort()
        .reverse()[0];
      setLastMarkerSyncTime(latestCreatedAt || new Date().toISOString());
      data.forEach(item => {
  ...
  const reportPoop = async () => {
    ...
    if (!reportError) {
      const reportPoints = selectedSize === 'L' ? 15 : selectedSize === 'M' ? 10 : 5;
      ...
      Alert.alert("Erfolg", `Haufen wurde markiert! +${reportPoints} XP 💩`);
    } else {
        ...
        <View style={{ flex: 1 }}>
              ...
              return (
                ...
                >
                  <View style={{ width: markerContainerSize, height: markerContainerSize, justifyContent: 'center', alignItems: 'center' }}>
                    <Text style={{ fontSize: markerSize, lineHeight: markerSize, textAlign: 'center' }}>💩</Text>
                  </View>
                </Marker>
              );
            })}
          ...
          {selectedPoop ? (
            ...
            <View style={styles.sizeRow}>
                {[
                  { id: 'S', label: '💩', visualSize: 22 },
                  { id: 'M', label: '💩', visualSize: 34 },
                  { id: 'L', label: '💩', visualSize: 46 }
                ].map(item => (
                  <View key={item.id} style={{alignItems: 'center'}}>
                    ...
                    <Text style={{marginTop: 8, fontWeight: 'bold', color: '#333', fontSize: 14}}>{item.id}</Text>
                  </View>
                ))}
              </View>
      ...
      {activeTab === 'Profil' && (
        <ScrollView style={styles.profileScroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={[styles.notificationSection, styles.shadow]}>
            ...
            <View style={styles.settingRow}>
              ...
              <Switch
                ...
                onValueChange={(value) => saveReportFeedbackSetting(REPORT_VIBRATION_STORAGE_KEY, value, setReportVibrationEnabled)}
          ...
          <View style={[styles.badgeSection, styles.shadow]}>
            <Text style={styles.badgeSectionTitle}>BADGES & MEILENSTEINE</Text>
            <View style={styles.badgeGrid}>
              {badgeDefinitions.map(badge => (
                <View key={badge.id} style={[styles.badgeCard, badge.achieved && styles.badgeCardActive, !badge.achieved && styles.badgeCardInactive]}>
                  <View style={[styles.badgeIconWrap, badge.achieved ? styles.badgeIconWrapActive : styles.badgeIconWrapInactive]}>
                    <Text style={[styles.badgeIcon, badge.achieved ? styles.badgeIconActive : styles.badgeIconInactive]}>{badge.icon}</Text>
                  </View>
                  <Text style={[styles.badgeTitle, badge.achieved ? styles.badgeTitleActive : styles.badgeTitleInactive]}>{badge.title}</Text>
